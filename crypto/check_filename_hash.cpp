...
BOOL checkForFileHash() {
	...
	if (!BCRYPT_SUCCESS(BCryptOpenAlgorithmProvider(
		...
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] Could not initialize hashing handle.\'}";
	...
	if (!BCRYPT_SUCCESS(BCryptGetProperty(
		...
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] Could not retrieve length for hashing object.\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
	...
	if (!BCRYPT_SUCCESS(BCryptCreateHash(
		...
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] Could not create hashing object.\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
	...
	if (!BCRYPT_SUCCESS(BCryptHashData(
		...
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] Could not hash file.\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
	...
	if (!BCRYPT_SUCCESS(BCryptFinishHash(
		...
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] Could not retrieve file hash.\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
    ...
    if (bufsizeCheck == 0) {
    	...
    	outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] Obtaining the correct size for the hex buffer failed.\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
...
int main(int argc, char* argv[]) {
	...
	if (checkForFile()) {
		...
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'SUCCESS\', \'output\': \'[INFO] ch0nky.txt file found!\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);

		if (checkForFileHash()) {
			//printf("ch0nky.txt hash found!\n");
			outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'SUCCESS\', \'output\': \'[INFO] ch0nky.txt hash found!\'}";
        	makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
		}
		else {
			//printf("ch0nky.txt hash not found...\n");
			outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] ch0nky.txt hash not found...\'}";
        	makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
		}
	}
	else {
		//printf("ch0nky.txt file not found...\n");
		outData = "{\'job_id\': \'0\', \'agent_id\': \'0\', \'command\': \'check_filename_hash.exe\', \'status\': \'ERROR\', \'output\': \'[ERROR] ch0nky.txt file not found...\'}";
        makeHttpRequestPOST("127.0.0.1", 5000, "/output", 0, outData);
	}
	return 0;
}